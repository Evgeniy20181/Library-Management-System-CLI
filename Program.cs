...
class Program
{
    ...
    static void Main()
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("10. Lette etter leseliste");
            Console.WriteLine("11. Avslutt");
            ...
            switch (choice)
            {
                ...
                case "9":
                    ...
                    break;
                case "10":
                    try { SearchReadingLists(); } catch (Exception ex) { Console.WriteLine($"Feil: {ex.Message}"); }
                    break;
                case "11":
                    return;
                default:
    ...
    static void SearchReadingLists()
    {
        Console.Write("Søk etter navn eller anledning: ");
        string? search = Console.ReadLine();

        using (var connection = new SQLiteConnection(connectionString))
        {
            connection.Open();
            string sql = "SELECT * FROM reading_lists WHERE name LIKE @search OR occasion LIKE @search";
            using (var command = new SQLiteCommand(sql, connection))
            {
                command.Parameters.AddWithValue("@search", "%" + search + "%");
                using (var reader = command.ExecuteReader())
